...
def search_train_vehicle(*, keyword: str, search_type: str, query_time: float):
    ...
    if local_results:
        ...
        for r in local_results:
            ...
            r['current_train_no'] = '暂无运行任务'
            r['route_time'] = '无时间信息'
            if not show_routes:
                continue
            model = (r.get('train_model_raw') or '').upper().replace('-', '')
            ...
            try:
                ...
                if not matched:
                    for item in data:
                        emu_no_api = item.get('emu_no', '').upper().replace('-', '')
                        if number in emu_no_api:
                            matched = item
                            break
                if matched:
                    current_train = matched.get('train_no', '').strip()
